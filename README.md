...
## Политика вежливости и капчи

> ⚠️ **Внимание.** Используйте ручное или внешнее решение капчи только для собственных сайтов или при наличии письменного разрешения владельца ресурса. Обход защиты чужих ресурсов запрещён.

CLI и web-версии используют модуль "politeness policy":

- запросы на один хост ограничены по частоте (0.2–0.5 rps) и выполняются последовательно;
- к каждому запросу добавляется случайная задержка 200–800 мс;
- повторные попытки для кодов 429/503/504 выполняются с учётом заголовка `Retry-After`;
- `robots.txt` парсится и кэшируется на 24 часа, учитываются `Disallow`, `Crawl-delay`, `Sitemap`;
- используется ротация белого списка `User-Agent`, а также заголовки `Accept-Language`, `Accept`, `DNT`;
- ответы с `ETag`/`Last-Modified` кешируются и переиспользуются.

При обнаружении виджетов reCAPTCHA/hCaptcha и т. п. клиент выбрасывает `ErrCaptchaRequiredException`. В dev-режиме CLI показывает диалог ручного ввода (`ConsoleCaptchaPrompt`). В CI всегда используется mock-режим (`CAPTCHA_PROVIDER=mock`), внешние провайдеры разрешены только при `ALLOW_EXTERNAL_CAPTCHA_SOLVER=true` и домене из `ALLOWED_CAPTCHA_DOMAINS`.

Подробнее — в [docs/captcha.md](docs/captcha.md).
